...
piece_init = convert_position([1 1 3], chess_scalar);
piece_fin = convert_position([2 4 3], chess_scalar);
...
full_route = move_piece(piece_init, piece_fin, REST);
...
function [route_vals] = parabolic_path(start_pos, end_pos)
    % Finds linear path between 2 positions
    ...
    disp(x_route)
    disp(y_route)
    z_route = 0.5*(1 - (4/(xs^2 +xf^2 -2*xf*xs + yf^2 + ys^2 -2*yf*ys))*((x_route - (xs + xf)/2).^2 + (y_route - (ys + yf)/2).^2))';
    disp(z_route)
    z_route = reshape(z_route, 51, 1);
    disp(z_route)
    %end
